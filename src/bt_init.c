...
void bt_create_task(void)
{
  ...
  //  bt_preread_local_address(local_public_addr);
    ...
    LOG_I(bt_spp_example, "[BT]local_public_addr [%02X:%02X:%02X:%02X:%02X:%02X]\n", bt_local_public_addr[5],
          bt_local_public_addr[4], bt_local_public_addr[3], bt_local_public_addr[2], bt_local_public_addr[1], bt_local_public_addr[0]);

    if (pdPASS != xTaskCreate(bt_task, BT_TASK_NAME, BT_TASK_STACKSIZE/(sizeof(StackType_t)), (void *)bt_local_public_addr, BT_TASK_PRIORITY, NULL)) {
